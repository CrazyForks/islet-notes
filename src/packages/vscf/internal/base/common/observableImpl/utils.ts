...
import { DisposableStore, IDisposable, toDisposable } from 'vscf/internal/base/common/lifecycle.ts';
import { autorun } from 'vscf/internal/base/common/observableImpl/autorun.ts';
import {
  IObservable,
  BaseObservable,
  transaction,
...
import { getLogger } from 'vscf/internal/base/common/observableImpl/logging.ts';
...
export function derivedObservableWithWritableCache<T>(
  name: string,
  computeFn: (reader: IReader, lastValue: T | undefined) => T,
): IObservable<T> & { clearCache(transaction: ITransaction): void } {
  let lastValue: T | undefined = undefined;
