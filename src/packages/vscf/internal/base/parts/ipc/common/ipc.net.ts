...
export class ConnectionHealthChangedEvent {
  constructor(public readonly connectionHealth: ConnectionHealth) {}
}
...
export class PersistentProtocol implements IMessagePassingProtocol {
  ...
  constructor(opts: PersistentProtocolOptions) {
    ...
    this._socketDisposables.add(
      this._socketLatencyMonitor.onSendLatencyRequest((buffer) =>
        this._sendLatencyMeasurementRequest(buffer),
      ),
    );
    this._socketDisposables.add(
      this._socketLatencyMonitor.onHighRoundTripTime((e) => this._onHighRoundTripTime.fire(e)),
    );
    this._socketDisposables.add(
      this._socketLatencyMonitor.onDidChangeConnectionHealth((e) =>
        this._onDidChangeConnectionHealth.fire(e),
      ),
    );
  ...
  private _sendLatencyMeasurementRequest(buffer: VSBuffer): void {
    this._incomingAckId = this._incomingMsgId;
    const msg = new ProtocolMessage(
      ProtocolMessageType.LatencyMeasurementRequest,
      0,
      this._incomingAckId,
      buffer,
    );
  ...
  private _sendLatencyMeasurementResponse(buffer: VSBuffer): void {
    this._incomingAckId = this._incomingMsgId;
    const msg = new ProtocolMessage(
      ProtocolMessageType.LatencyMeasurementResponse,
      0,
      this._incomingAckId,
      buffer,
    );
...
class LatencyMonitor extends Disposable {
   ...
   */
  private _latencySamples: number[] = Array.from(
    { length: ProtocolConstants.LatencySampleCount },
    (_) => 0,
  );
  ...
  public handleResponse(buffer: VSBuffer): void {
    ...
    const highLatencySampleCount = this._latencySamples.filter(
      (s) => s >= ProtocolConstants.HighLatencyTimeThreshold,
    ).length;
    this._connectionHealth =
      highLatencySampleCount >= ProtocolConstants.HighLatencySampleThreshold
        ? ConnectionHealth.Poor
        : ConnectionHealth.Good;

    if (roundtripTime > ProtocolConstants.HighLatencyTimeThreshold) {
      this._onHighRoundTripTime.fire(
        new HighRoundTripTimeEvent(roundtripTime, highLatencySampleCount),
      );
    }
