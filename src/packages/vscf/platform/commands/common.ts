// @ts-nocheck
export { ICommandService, CommandsRegistry } from 'vs/platform/commands/common/commands';

export type { ICommandHandler, ICommandEvent } from 'vs/platform/commands/common/commands';

export { AbstractKeybindingService } from 'vs/platform/keybinding/common/abstractKeybindingService';
export { KeybindingResolver } from 'vs/platform/keybinding/common/keybindingResolver';
